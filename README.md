...
`tools/list` at `https://gateway.pipeworx.io/httpcat/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/get_status_cat \
  -H 'Content-Type: application/json' \
  -d '{"status_code":200}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/get_status_cat`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
